...
tmux send-keys -t $SESSION_NAME:0.2 "CUDA_VISIBLE_DEVICES=2 vllm serve BAAI/bge-reranker-v2-m3 --port 8003 --dtype float16 --gpu-memory-utilization 0.3" C-m

# Pane 0.3: Deepseek OCR (GPU 2)
tmux send-keys -t $SESSION_NAME:0.3 "$SETUP_CMD" C-m
tmux send-keys -t $SESSION_NAME:0.3 "export VLLM_USE_TRITON_FLASH_ATTN=0 && export VLLM_ATTENTION_BACKEND=FLEX_ATTENTION && CUDA_VISIBLE_DEVICES=2 vllm serve deepseek-ai/DeepSeek-OCR --port 8004 --dtype float16 --gpu-memory-utilization 0.6 --max-model-len 8192 --logits-processors NGramPerReqLogitsProcessor --mm-processor-cache-gb 0 --no-enable-prefix-caching" C-m
