...
def clean_email_llm(email_text:str, prompt, model:AutoModelForCausalLM, tokenizer: AutoTokenizer, trace_name:str, device:torch.device) -> str:
    ...
    try:
        with trace(
            ...
            assert input_ids.max() < model.config.vocab_size, f"Token ID exceeds model vocab size: {input_ids.max()}, {model.config.vocab_size}"
            print("token_count:",token_count+100)
