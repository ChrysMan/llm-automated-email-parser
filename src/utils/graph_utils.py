...
def clean_data(text: str) -> str:
    ...
    clean_text = re.sub(r"Tel\s*:\s*.+$|^(T|M)\s*:\s*\+*.+$|^E(-)?mail\s*:.*$|Website\s*:.*$|Web\s*: .+$|Address\s*:.+$|Fax\s*:.+$|P\.*s\.*\s*:.+$|mob\.\+*.+\s*$|Mobile\s*:.+$|Phone\s*:\s*.*$|Disclaimer\s*:.+$|^Στάλθηκε από το Ταχυδρομείο.+$|^Sent from my.+$|地址\s*:.+$|分公司\s*:.+$|This message is sent from my mob* device|^<htt*", "", clean_text, flags=flags)
    clean_text = re.sub(r"\n\s*\n*", "\n", clean_text)
    return clean_text
