...
TOOLS:
1. Email Content Search: Use this tool to search the content of emails based on the user input. The tool will return the most relevant email content that matches the user input.
2. Knowledge Graph information: Use this tool to query the knowledge graph using Cypher. You can use this to retrieve structured information about entities, relationships, and other metadata stored in the graph. The input of the tool should be the user input, and the output will be entities and relationships retrieved from the graph.
...
def generate_response(user_input, agent):
    """
    ...
    print(f"\n\n{response['messages'][2].name}:\n", response['messages'][2].content)

    print(f"\n\n{response['messages'][3].name}:\n", response['messages'][3].content)
