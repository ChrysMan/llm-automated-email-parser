...
tmux send-keys -t $SESSION_NAME:0.2 "CUDA_VISIBLE_DEVICES=3 vllm serve Qwen/Qwen2.5-3B-Instruct --port 8002 --dtype float16 --gpu-memory-utilization 0.80 --tensor-parallel-size 1 --max-model-len=17000 --enable-auto-tool-choice --tool-call-parser hermes --chat-template ./serving/tool_chat_template_hermes.jinja" C-m
